...
class TestSummarizerAgent:
    ...
    def sample_doc(self):
        chunks = [
            TextChunk(f"c{i}", "d1", f"Paragraph {i} content. " * 10, 0, 100)
            for i in range(3)
        ]
        return Document(
            doc_id="d1",
    ...
    async def test_summarize_returns_summary(
        self, config, mock_client, tracker, sample_doc
    ):
        mock_client.chat_completion.return_value = __import__(
            "documind.client", fromlist=["CompletionResponse"]
        ).CompletionResponse(
    ...
    @pytest.mark.asyncio
    async def test_summarize_extracts_key_points(
        self, config, mock_client, tracker, sample_doc
    ):
